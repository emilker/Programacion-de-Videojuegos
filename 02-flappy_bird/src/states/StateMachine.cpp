...
void StateMachine::change_state(const std::string& state_name, std::shared_ptr<World> world, std::shared_ptr<Bird> bird, bool log, bool powerup, int score, float time) noexcept
{
    ...
    current_state->enter(world, bird, log, powerup, score, time);
}
