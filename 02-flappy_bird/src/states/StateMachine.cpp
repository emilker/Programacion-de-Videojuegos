...
void StateMachine::change_state(const std::string& state_name, std::shared_ptr<World> world, std::shared_ptr<Bird> bird, bool pause, bool timer_powerup, int score, float _time) noexcept
{
    ...
    current_state->enter(world, bird, pause, timer_powerup, score,_time);
}
